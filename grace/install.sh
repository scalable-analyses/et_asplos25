...
scons libtorch=../venv_pytorch/lib/python3.9/site-packages/torch blas=yes tblis=$(pwd)/../tblis libxsmm=$(pwd)/../libxsmm -j8
mv build build_gcc

CXX=clang++ CC=clang scons libtorch=../venv_pytorch/lib/python3.9/site-packages/torch blas=yes tblis=$(pwd)/../tblis libxsmm=$(pwd)/../libxsmm -j8
mv build build_llvm
cd ..